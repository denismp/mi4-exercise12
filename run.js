...
function sleep() {
    timeoutID = setTimeout(clearTimeOut, 5000, 'setTimeout Demo!');
}
...
function addCandidate(candidate, contractInstance, accounts) {
    addTristanHash = contractInstance.methods.addCandidate(candidate).send({ from: accounts[1] })
            .then(
                result => {
                    console.log(result);
                    candidateAdded = true;
                    sleep();
                    // contractInstance.methods.voteForCandidate(candidate).send({ from: accounts[2] }).then(result => console.log("Vote for Tristan=" + result));
                });
}

function voteForCandidate(candidate, contractInstance, account) {
    contractInstance.methods.voteForCandidate(candidate).send({ from: account })
        .then(
            result => { console.log("Vote for " + candidate + "=" + result);
            voted = true;
            }
        );
}

// async function sleep(flag) {
//     while( flag === false ) {
...
async function test() {
    ...
    while( candidateAdded === false ) {
       await new Promise(r => setTimeout(r, 5000));
    }
    candidateAdded = false;
    addCandidate("Rave", contractInstance, accounts);
    while( candidateAdded === false ) {
       await new Promise(r => setTimeout(r, 5000));
    }
    candidateAdded = false;
    // voteForCandidate("Tristan", contractInstance, accounts[2]);
    // while (voted == false) {
    //    await new Promise(r => setTimeout(r, 5000));
    // }
    // voted = false;
